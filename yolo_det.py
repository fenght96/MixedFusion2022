...
class Net():
    def __init__(self, weights='best.pt',
                 view_img=False, imgsz=640, device='cuda:0',
        ...
        # Load model
        self.model = YOLOv5(self.weights, device).model
        #attempt_load(self.weights, map_location=self.device)  # load FP32 model
        self.model.eval()
        if self.half:
    ...
    def detect(self, im0s):
        ...
        if len(pred):
            ...
            for *xyxy, conf, cls in reversed(pred):
                xyxy = torch.tensor(xyxy).int().view(-1).tolist()
