...
parser.add_argument('--dataset_root', type=str, default = '/home/fht/data/ocrtoc/', help='dataset root dir (''YCB_Video_Dataset'' or ''Linemod_preprocessed'')')
...
def main():
    ...
    if opt.dataset == 'ycb':
        opt.num_objects = 33 #number of object classes in the dataset
    ...
    #estimator.load_state_dict(torch.load('./ycb_model.pth'))

    if opt.resume_posenet != '':
        '''
        weights = torch.load('{0}/{1}'.format(opt.outf, opt.resume_posenet))

        del weights['conv4_r.weight']
        del weights['conv4_r.bias']
        del weights['conv4_t.weight']
        del weights['conv4_t.bias']
        del weights['conv4_c.weight']
        del weights['conv4_c.bias']
